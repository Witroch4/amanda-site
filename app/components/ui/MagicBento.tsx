...
export function MagicBentoCard({
    ...
    const baseClassName = `magic-card flex flex-col justify-between relative w-full p-6 rounded-[20px] border border-solid overflow-hidden transition-all duration-300 ease-in-out hover:-translate-y-0.5 hover:shadow-[0_8px_25px_rgba(0,0,0,0.15)] ${enableBorderGlow ? 'magic-card--border-glow' : ''
        } ${className}`;
    ...
    return (
        <>
          ...
          .magic-card--border-glow:hover {
            box-shadow: 0 4px 20px rgba(212, 175, 55, 0.15), 0 0 30px rgba(${glowColor}, 0.2);
          }
        ...
        `}
            ...
            <div ref={gridRef} className="magic-bento-section">
                {children}
            </div>
        </>
    );
