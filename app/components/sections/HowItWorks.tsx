...
function CheckIcon({ isVisible }: { isVisible: boolean }) {
    return (
        <div
            className={`w-12 h-12 rounded-full border-4 border-gold-400 bg-red-900 flex items-center justify-center transition-all duration-500 ${isVisible ? "scale-100 opacity-100" : "scale-0 opacity-0"
                }`}
        >
            <svg
                className={`w-6 h-6 text-gold-400 transition-all duration-300 delay-200 ${isVisible ? "scale-100 opacity-100" : "scale-0 opacity-0"
                    }`}
                fill="none"
                viewBox="0 0 24 24"
                stroke="currentColor"
    ...
    return (
        <div className="flex gap-6 md:gap-8 relative">
            ...
            <div className="flex-shrink-0 relative z-10">
                <CheckIcon isVisible={isVisible} />
            </div>

            {/* Content */}
            <div
                className={`flex-1 pb-12 transition-all duration-700 delay-300 ${isVisible ? "translate-x-0 opacity-100" : "translate-x-10 opacity-0"
                    }`}
            >
                    ...
                    <div className="space-y-0">
                        {steps.map((step, index) => (
                            <div key={index} ref={(el) => { stepRefs.current[index] = el; }}>
                                <StepCard step={step} index={index} isVisible={visibleSteps[index]} />
                            </div>
                        ))}
                    </div>
                </div>
